...
namespace sim_odom_controller{
    ...
    public:
        ...
        SimOdomController() : ugv_normal_(0), ugv_initial_yaw_(0), ugv_initial_omega_(0), kp_(10), kd_(1), nh_("~"), ugv_odom_frame_("/robot_1/base_footprint")
        {
            ...
            uav_coef_sub_ = nh_.subscribe("uav_coef", 10, &SimOdomController::uav_coef_call_back, this);
            ugv_coef_sub_ = nh_.subscribe("ugv_coef", 10, &SimOdomController::ugv_coef_call_back, this);

            uav_pose_sub_ = nh_.subscribe("uav_pose", 1000, &SimOdomController::uav_pose_call_back, this);
	...
	bool flag_is_sim_;
        ...
        bool is_traj ;
	bool is_first;
	double traj_time_offset;
    bool single_odom_rcv;
        ...
        void uav_coef_call_back(const frontier_exploration::MultiArrayWithHeader &msg){
            if(uav_plan_time>=msg.header.stamp.toSec()-0.01){
                ROS_INFO("Same time UAV Coef");
                return;
            }
        ...
        void uav_pose_call_back(const nav_msgs::Odometry &msg) {
            ...
            nav_msgs::Odometry m = msg;
            //transformOdom(m, "");
            if(single_odom_rcv) { single_odom_rcv=0; return;}
                this->single_odom_rcv=1;
            if (is_init) {
                if(is_traj) {
                    double dT = m.header.stamp.toSec();
                    double des_x = 0;
			...
			if(is_first) {
                received_msg=0;
				traj_time_offset=dT-uav_plan_time;
				is_first= false;
                ROS_ERROR("uav_mat_received, time is %f", dT);

			}
            
            //dT-=traj_time_offset;
            //received_msg++;
            //cout<<"reveive_msg:"<<received_msg<<endl;
                    for (int i = 0; i < uav_t_.size(); i++) {
                        //cout<<i<<endl;
                        //cout<<dT<<endl;
                        //cout<<uav_t_<<endl<<endl;
                        if (dT < uav_t_(i)) {
			    ...
			    des_ay = t_a * coef_y;
                    ...
                    nav_msgs::Odometry des_pos;
					des_pos.header.stamp    = m.header.stamp;
