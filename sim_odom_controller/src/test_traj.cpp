...
void pub_cmd(const nav_msgs::Odometry& msg)
{
    ...
    {
        cout<<"[#INFO] is_init true in pub_cmd"<<endl;
        is_init = true;
    }
}
...
void body_pose_callback(const nav_msgs::Odometry &msg)
{
    ...
    if(point_pass<3)
    {
        ...
        else
        {
            ...
            cout<<"time consume in solver : "<<(time1 - time0).toSec()<<endl;
            ...
            cout<<"time consume : "<<(time2 - time0).toSec()<<endl;
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle nh;
    body_pose_sub = nh.subscribe("/vins_estimator/imu_propagate", 10, body_pose_callback);
